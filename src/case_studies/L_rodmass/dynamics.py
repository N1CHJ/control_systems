...
class RodMassDynamics(DynamicsBase):
    ...
    def f(self, x, u):
        ...
        # Ensure u is an array and extract torque
        u = np.atleast_1d(u)
        tau = u[0]

        # Calculate equation of motion
        # theta_ddot = (1 / (m * ell^2)) * (tau - b*thetadot - m*g*ell*cos(theta) - k1*theta - k2*theta^3)
        theta_ddot = (1.0 / (self.m * self.ell**2)) * (
            tau - self.b * thetadot - self.m * self.g * self.ell * np.cos(theta) - self.k1 * theta - self.k2 * theta**3
        )

        # Build state derivative vector
        xdot = np.array([thetadot, float(theta_ddot)])
        return xdot

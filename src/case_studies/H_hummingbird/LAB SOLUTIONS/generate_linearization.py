...
# BEGIN SOLUTION
qddot = qdot.diff(t)
phiddot, thetaddot, psiddot = qddot
B_mat = sp.eye(3, 3) * beta
LHS = M @ qddot + C + dP_dq
...
# BEGIN SOLUTION
eom_lon = sp.Eq(eom.lhs[1], eom.rhs[1])
# END SOLUTION
...
eom_lon = eom_lon.subs(lon_decouple_subs)
# END SOLUTION
su.printsym(eom_lon)
...
# BEGIN SOLUTION
gravity_eom = eom_lon.subs({thetadot: 0, thetaddot: 0})
F_fl = sp.solve(gravity_eom, F)[0]
# END SOLUTION
...
# BEGIN SOLUTION
thetaddot_sol = sp.solve(linearized_eom_lon, thetaddot)
linearized_eom_lon = sp.Eq(thetaddot, thetaddot_sol[0])
# END SOLUTION
...
# BEGIN SOLUTION
thetaddot_eom = linearized_eom_lon.rhs.subs({beta: 0})
thetaddot_eom = sp.simplify(thetaddot_eom)
# END SOLUTION
...
# BEGIN SOLUTION
lat_decouple_subs = {
    theta: 0, thetadot: 0, thetaddot: 0,
    F: F_eq_expr,
    beta: 0,
}
eom_lat = eom_lat.subs(lat_decouple_subs)
# END SOLUTION
su.printsym(eom_lat)
...
# BEGIN SOLUTION
lat_accels = sp.solve(eom_lat, [phiddot, psiddot])
phiddot_eom = lat_accels[phiddot]
psiddot_eom = lat_accels[psiddot]

xdot_lat = sp.Matrix([phidot, psidot, phiddot_eom, psiddot_eom])
x_lat = sp.Matrix([phi, psi, phidot, psidot])
u_lat = sp.Matrix([T])

A_lat = xdot_lat.jacobian(x_lat)
B_lat = xdot_lat.jacobian(u_lat)

lat_eq_vals = {phi: 0, psi: 0, phidot: 0, psidot: 0, T: 0}
# END SOLUTION
